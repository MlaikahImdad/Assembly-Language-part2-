...
start:          mov [paramblock+0], cs          ; code segment parameter
                ...
                mov ah, 0x4a                    ; service 4a – resize memory
                mov bx, end                     ; end of memory retained
                add bx, 15                      ; rounding up
                mov cl, 4
                shr bx, cl                      ; converting into paras
                int 0x21                        ; dos services

                mov ah, 0x4b                    ; service 4b - exec
                mov al, 0                       ; load and execute
                mov dx, command                 ; command to be executed
                mov bx, execblock               ; address of execblock
                mov word [bx+2], tail           ; offset of command tail
                mov [bx+4], ds                  ; segment of command tail
                int 0x21                        ; dos services
                
                jmp $                           ; loop infinitely if returned